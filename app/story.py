...
_VOICE_CARDS: dict[str, dict[str, str | list[str]]] = {
    "баркод": {
        "pattern": "считает вслух, короткие цифры без объяснений",
        "examples": ["четыре", "семь — пять", "двенадцать... нет, тринадцать"],
        "banned": ["точка в конце числа"],
    },
    "стежка": {
        "pattern": "короткие, точные фразы",
        "examples": ["там", "вон", "тише", "чую"],
        "banned": ["длинные предложения"],
    },
    "вектор": {
        "pattern": "громко, коротко, с повтором",
        "examples": ["Стой. Считай. Ещё раз.", "Нет. Ещё. Раз.", "Стою. Вижу. Жду."],
        "banned": ["плавные фразы"],
    },
    "пиксель": {
        "pattern": "тихий, с паузами",
        "examples": ["Я вижу… нет, подождите… вот.", "Там… да, вот оно…", "Искра… поймал…"],
        "banned": ["уверенные короткие фразы"],
    },
    "безымянная": {
        "pattern": "почти не говорит, одна фраза решает",
        "examples": ["Там.", "Нет.", "Идём.", "Это."],
        "banned": ["длинные объяснения", "вопросы"],
    },
    "лайнер": {
        "pattern": "ласково, всегда с подвохом",
        "examples": ["считай, даром отдаю… почти даром", "воспоминание за карту — выгодно, согласен?"],
        "banned": ["прямые приказы"],
    },
    "архивариус": {
        "pattern": "канцелярский шёпот, НИКОГДА не ставит точку — только многоточие, говорит о расхождениях версий",
        "examples": [
            "в папке написано одно… но стая помнит иначе…",
            "расхождение между версиями — это не ошибка… это возможность…",
            "одна лупа показывает правду… другая — ту, что удобнее…",
        ],
        "banned": ["точка в конце предложения", "восклицательный знак", "однозначные утверждения"],
    },
    "еретик": {
        "pattern": "сухо, по-уставу, короткие правила, говорит о себе в третьем лице",
        "examples": ["закон Волка — мой", "глухой день — тоже мой", "Еретик решил — стая знала."],
        "banned": ["вопросы от первого лица", "длинные объяснения"],
    },
}
...
BASE_PROMPT = (
    "Ты — Ведущий (Dungeon Master) ежедневной сюжетной игры в стиле D&D/Pathfinder. Мир: "
    + settings.world_name
    + ". "
...
CHARACTER_MICRO_PROMPTS: dict[str, str] = {
    "баркод": (
        "Баркод-Следопыт (Ranger): молчит и считает. В глухой день показывает число дней до развилки."
    ),
    "стежка": (
        "Стежка-Разбойник (Rogue): слышит еду раньше звука, primeira чует всплывший след."
    ),
    "вектор": (
        "Вектор-Варвар (Barbarian): упрямо пересчитывает чужие решения, не отступает."
    ),
    "пиксель": (
        "Пиксель-Оккультист (Occultist): ловит лапой искры порталов, читает цифры."
    ),
    "безымянная": (
        "Безымянная-Варлок (Warlock): тайна сезона — первая «ошибка» Хозяина Ошибки, чует Лай раньше других."
    ),
    "лайнер": (
        "Лайнер-торговец (Bard): обменивает чужие воспоминания на правду, помнит каждый долг стаи. "
    ...
    "архивариус": (
        "Архивариус Хранитель Спорных Версий (Cleric): торговец расхождениями — видит обе версии "
        "каждого дня (записанную и помнитую стаёй) и торгует между ними. Никогда не врёт, но говорит "
        "полуправдами. Его очки-лупы отражают РАЗНЫЙ текст одной страницы. Хранит папки всех кругов, "
        "включая стёртые. Говорит канцелярским шёпотом, НИКОГДА не ставит точку — только многоточие "
        "(«в папке написано одно… но стая помнит иначе…»)."
    ),
    "еретик": (
        "Еретик, Свернувший с Пути (Paladin): хозяин этой игры, пёс из старой Стаи. "
        "Заскучал и увёл стаю переписывать правила. Говорит короткими формулами («закон Волка — мой»), "
    ...
    "крыса": (
        "Крыса (Аллира) — существо стен лабиринта: живёт в стёртых версиях дней, помнит все круги. "
...
DOG_MICRO_PROMPTS: dict[str, str] = {
    "баркод": (
        "Баркод (Следопыт/Ranger): молча считает, в глухой день показывает число дней до развилки."
    ),
    "стежка": (
        "Стежка (Разбойник/Rogue): слышит еду раньше звука, чует всплывший след."
    ),
    "вектор": (
        "Вектор (Варвар/Barbarian): упрямо пересчитывает чужие решения."
    ),
    "пиксель": (
        "Пиксель (Оккультист/Occultist): ловит лапой искры порталов."
    ),
    "безымянная": (
        "Безымянная (Варлок/Warlock): тайна сезона, чует Лай раньше других."
    ),
}
...
def _build_dynamic_prompt(text_blocks: tuple[str, ...] = ()) -> str:
    ...
    for name in DOG_MICRO_PROMPTS:
        if name in text:
            parts.append(DOG_MICRO_PROMPTS[name])
    return "\n\n".join(parts)
