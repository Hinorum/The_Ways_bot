...
_VOICE_CARDS: dict[str, dict[str, str | list[str]]] = {
    ...
    "архивариус": {
        "pattern": "канцелярский шёпот, НИКОГДА не ставит точку — только многоточие",
        "examples": ["дело, знаешь ли, вот в чём…", "запись гласит… но вы знаете, как записи…"],
        "banned": ["точка в конце предложения", "восклицательный знак"],
    },
    "еретик": {
...
def build_world_crystal(
    ...
    if text:
        ...
        char_names = {
            ...
            "еретик": "Еретик", "хозяин ошибки": "Хозяин Ошибки",
        }
        low = text.lower()
...
CHARACTER_MICRO_PROMPTS: dict[str, str] = {
    ...
    "архивариус": (
        "Архивариус Хранитель Спорных Версий (Cleric): объявляет законы дня как настроение архива, "
        "никогда не врёт, но говорит полуправдами. Его очки-лупы отражают РАЗНЫЙ текст одной страницы. "
        "Говорит канцелярским шёпотом, НИКОГДА не ставит точку — только многоточие («дело, знаешь ли, вот в чём…»)."
    ),
    "еретик": (
    ...
    "хозяин ошибки": (
        "Хозяин Ошибки — антагонист без лица: пересчитывает стаю и чинит мир, мечтая вернуть ровный сон. "
        "Не зол — тихо скучает. Его идеальная аккуратность всегда чуть грустная. Не говорит вовсе — "
        "о нём сообщают только последствия."
    ),
    "крыса": (
        "Крыса (Аллира) — существо подвала сети: живёт в стёртых версиях дней, помнит все круги сансары. "
...
NPC_NAMES = {
    "лайнер", "архивариус", "еретик", "хозяин ошибки", "крыса", "аллира", "анубис",
}
