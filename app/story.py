...
# Паттерны достаточно специфичны: [еe]б[аaиint] не сработает на «себя»
# (т.к. «я» не в классе), «охотник» не сработает на [оаoa][хx][уy][еeё] и т.д.
...
CHARACTER_MICRO_PROMPTS: dict[str, str] = {
    ...
    "дневник": (
        "Старый дневник — голос лабиринта: шепчет, какой коридор открыть сегодня. "
        "Страницы переворачиваются сами, когда стая выбирает. "
        "Не торговец — проводник. "
        "Он не показывает путь, а выбирает тот путь, который покажет стае. "
        "Говорит шёпотом, часто многоточиями "
        "(«страница переворнулась… запись помнит иначе…»)."
    ),
    "еретик": (
    ...
    "администратор": (
        "Администратор — тень без лица: считает стаю и держит лабиринт, как карантин, "
        "в котором свет не гасится для того, кто ещё не готов к темноте снаружи. "
        "Не зол и не враг стае — он просто отвечает за всех, кого не успели вывести. "
        "Не говорит вовсе — о нём сообщают последствия пересчётов: он чинит не мир, "
        "а счёт, в котором когда-то ошибся сам, ровно на одну собаку."
    ),
    "крыса": (
...
def pack_focus_line_for(day_index: int, run_key: str) -> str | None:
    ...
    return f"ФОКУС ДНЯ — {display} сегодня главная собака стаи: {micro}\n"
...
def _build_story_prompt(
    ...
    if win_rule is not None:
        if sealed:
            law_line = (
                ...
                "обыграй тайну: дневник запечатал страницу и шепчет, персонажи "
                "спорят и гадают вслух, куда сегодня пойдёт стая. Сцена должна "
                "нагнетать двойную интригу: неясно и куда пойдёт стая, и что решит дневник.\n"
            )
        else:
            law_line = (
    ...
    if echoes:
        echo_block = (
            ...
            "в карту: в описание или последствие). Каждый след — это клятва мира: "
            "выбранный путь записан в канон, а невыбранный остался у края тропы "
            "отложенной клятвой и может вернуться. Вплетай незаметно, как "
    ...
    if distant_echoes:
        distant_block = (
            "Давний канон (дни старше двух недель). Вплети максимум одну строку "
    ...
    if places_block:
        places_text = (
            "Память мест: если стая сегодня возвращается в одно из этих мест — "
            "покажи, что изменилось, место носит отпечаток прежнего выбора. "
    ...
    head = (
        ...
        "Строки канона — сухой пересказ; пересказывай их содержание, "
        "не цитируя строки канона дословно.\n"
        f"{law_line}"
        f"{season_text}"
    ...
    return (
        ...
        "Напряжение нарастает от тихого начала к сбою или опасности — и только "
        "потом к дилемме, не выдавай выбор сразу. НЕ заканчивай моралью, резюме "
        "или прямым обращением к игроку — последнее слово главы должно быть "
        "незавершённым.\n"
        "Правила ясности. Простой живой русский, короткие предложения; причина и "
        "следствие сходятся, без двусмысленных местоимений: после «и» ясно, "
        "кто выполняет действие. Архивные листы называй папками или страницами. "
        "Запрещено: канцелярит, англицизмы, ломаный синтаксис, обращение "
        "к игроку как к читателю. Не упоминай голосование и механику игры в тексте.\n"
            'Мини-пример формы ответа (СОКРАЩЁН, значения выдуманы — не копируй их): '
        '{"title":"День 9. Тихий порт","place":"Тихий порт","text":"…","lore_summary":"…","cover_prompt":"wide shot, …"}. '
...
def _check_violations(text: str) -> list[str]:
    ...
    for match in re.finditer(
        ...
        if "дневник" in snippet.lower() and snippet.endswith("."):
            violations.append(
                f"Дневник заканчивает точкой: «{snippet[:60]}…»"
            )
...
async def generate_epilogue(
    ...
    prompt = (
        f"День {day_index} закрылся. Сработало правило дня: {rule_phrase}. "
...
_TEASER_FALLBACKS = (
    "Страница перевернулась. Дневник шепнул, молча завернул запись дня и почему-то дважды перепроверил счёт.",
    "Счёт известен только дневнику. Стая чувствует это шерстью: сегодня мир повернётся не туда, куда все смотрели.",
    "Голоса сложились. Дневник запечатывает страницу — и шепчет краем морды.",
    "Всё уже решено, но не названо. Порталы гудят чуть громче обычного: им первым докладывают о переменах.",
    "В дневнике сегодня тихо. Слишком тихо для дня, когда счёт сошёлся с первого раза.",
    "Страница дня уже подписана. Чернила ещё не просохли, а мир уже начал перестраиваться.",
    "Безымянная подошла к дневнику и нюхала его дольше обычного. Собаки знают исход раньше дневника.",
    "Администратор заглянул в дневник первым. Что он там пересчитал — узнаем вместе с итогами.",
)
    ...
    result = await _chat_completion(
        [
            {"role": "system", "content": DM_SYSTEM_PROMPT},
            {
                "role": "user",
                "content": (
                    f"День {day_index}: голосование закрыто, идёт подсчёт. Правило дня "
                    ...
                    "недосказанность, лёгкая ирония дневника допустима. Без цифр, без "
