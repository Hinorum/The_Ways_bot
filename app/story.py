...
def _build_story_prompt(
    ...
    return (
        ...
        "бросок, новых главных персонажей не вводи. Строй рассказ тремя "
        "движениями: (1) предыстория — чем отозвался вчерашний выбор из канона, "
        "что он изменил в мире и в стае; (2) сцена сейчас — куда стая вышла "
        "сегодня и что происходит; одна-две конкретные чувственные детали "
        "(звук портала, свет мисок, шёпот архива); (3) напряжение выбора — что "
        "случилось этим утром и что стае нужно решить до заката. Три карты — "
        "трудная дилемма: риск против заботы против хитрости, без очевидно "
        "правильного ответа.\n"
        "Правила ясности. Пиши простым живым русским языком, короткими "
        "предложениями; причина и следствие обязаны сходиться. Каждая карта — "
        "одно конкретное действие с понятной ценой: что стая сделает и чем "
        "рискует. Запрещено: вставлять одну и ту же фразу или название места "
        "во все три карты; повторять формулировки главы в картах дословно; "
        "канцелярит, англицизмы, ломаный синтаксис, случайная латиница в "
