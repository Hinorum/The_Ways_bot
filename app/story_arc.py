...
_ARC_STAGES: tuple[dict, ...] = (
    {
        ...
        "missions": (
            ...
            "У двери стая нашла пустую миску, расставленную строго по кругу: место готово под всех.",
        ...
        "teaser": (
            ...
            "Кость брошена, и даже двери притихли: кто-то ждёт, чем стая подтвердит свой первый шаг.",
        ),
    ...
    },
    {
        "index": 1,
        "name": "Поиск",
        "purpose": "стая ищет Сердце лабиринта; понимает, кто такой Администратор и зачем он считает",
        ...
        "whisper": (
            ...
            "Вечер вышел длиннее: лабиринт ждал, что стая дорисует карту до конца.",
    ...
    },
    {
        "index": 2,
        "name": "Кризис",
        "purpose": "лабиринт ломается; стая у двери к Сердцу, Администратор выходит из тени",
...
async def _generate_season_stage_via_llm(stage_index: int, season: int, llm_caller) -> dict | None:
    ...
    if isinstance(data, dict) and all(k in data for k in ("name", "purpose")):
        return {
            "name": data["name"][:80],
            "purpose": data["purpose"][:300],
            "tone": data.get("tone", "")[:200],
            "missions": [m[:150] for m in data.get("missions", [])],
            "whisper": [w[:150] for w in data.get("whisper", [])],
            "teaser": [t[:150] for t in data.get("teaser", [])],
            "guest": data.get("guest", "")[:200],
        }
