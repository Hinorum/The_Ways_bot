...
def _bot_description() -> str:
    base = (
        "Ты — голос стаи потерянных собак в лабиринте нестабильных коридоров.\n"
        "Каждое утро Архивариус выносит три тропы и объявляет закон дня: "
        "большинство, меньшинство или середина. Победивший путь становится "
        "каноном, и завтрашняя глава вырастет из него.\n"
        "Мир помнит всё: даже невыбранные тропы оставляют след. В стенах — "
        "Крыса, память стёртых кругов. В последний день месяца у двери встаёт "
        "Анубис, судья цикла: он взвешивает счёт против выбора.\n"
    )
    hints = []
