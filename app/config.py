...
class Settings(BaseSettings):
    ...
    # pack_state/needs остаются в коде (историческая совместимость), но не
    # обновляются и не влияют на игру. Ниже — флаги сублистем, генеривших
    # шум и отвлекавших сюжет.
    # Шрамы мира: новые шрамы не создаются (прошлые, если были, остаются в БД).
    world_scars: bool = False
    # Эмоциональный профиль стаи (усталость/надежда/паранойя) не двигается.
    emotion_system: bool = False
