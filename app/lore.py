...
def compose_chapter(
    ...
    _floor_pads = (
        ...
        "сеть считала и её.",
        "Тени легли к картам, хотя света уже не было — здесь тень умеет читать "
        "быстрее лап.",
        "Чужой ветер принёс запах страницы, которую ещё не написали; Пиксель "
        "чихнул дважды.",
        "Портал мигнул не в такт остальным, будто кто-то далеко перелистнул их "
        "день заранее.",
        "Баркод провёл когтем по пыли черту и сел рядом: цифры сегодня подождут "
        "развилки.",
        "Стежка обошла миски по кругу трижды и села ровно напротив середины — "
        "так ей спокойнее.",
        "Вектор начал пересчитывать тропы вслух и сбился на второй: даже "
        "упрямство умеет молчать.",
        "Безымянная вернулась с края поляны с чужим ошейником в зубах — "
        "пустым, но ещё тёплым.",
    )
    _min_len = 800
    _pad_i = int(hashlib.sha256(f"pad:{title}".encode()).hexdigest()[:8], 16)
    _used = 0
    while len(text) < _min_len and _used < len(_floor_pads):
        pad = _floor_pads[_pad_i % len(_floor_pads)]
        _pad_i += 1
        _used += 1
        if pad not in text:
            text += f" {pad}"
