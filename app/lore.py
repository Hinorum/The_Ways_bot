...
WORLD_BIBLE = (
    "Мир называется Эхо Стаи — сеть нестабильных порталов, где каждый мир собран из чужих решений. "
    ...
    "какой путь побеждает по итогам. Где-то в глубине сети звучит Первый Лай: дорога домой или "
    "ловушка — не знает никто. По следу стаи идёт Хозяин Ошибки из старого мира: он мечтает вернуть "
    "всем ровный сон, в котором счёт сходится и ничего не ошибается. Еретик построил эту игру, "
...
ARCHIVE_ORIGIN = (
    ...
    "и привёл в сеть миров, где один день — одно решение на всех, и мир "
    "перестраивается под твой выбор…\n\n"
    "По следу стаи идёт Он, Хозяин Ошибки, из того же старого сна… он не злой, "
    "он просто боится свободы и хочет вернуть всем ровный сон без ошибок… а "
    "Лайнер продал уже всю свою память и теперь торгует чужими снами и молчанием "
    "своего радио…\n\n"
    "Где-то в глубине сети звучит Первый Лай… дорога домой или приманка — не "
...
_RISK_PATHS = [
    ...
    (
        "Взять удар",
        "Встать между портальным разрядом и стаей, приняв весь удар на себя.",
    ...
    (
        "Двойной портал",
        "Войти в портал, который ведёт в оба мира одновременно — и выбрать один навсегда.",
        "Мир выбран. Второй мир растворился — и всё, что в нём было, теперь только в памяти.",
    ),
    (
        "Разбудить стража",
        "Лаять на спящего стража портала, пока он не покажет лицо — даже если лицо окажется твоим.",
    ...
    (
        "Последний рывок",
        "Бежать навстречу закрывающемуся порталу, зная: если стая войдёт целиком — мир изменится навсегда.",
        "Портал закрылся за последней собакой. Стая цела, но одна из них теперь считает дни по-новому.",
    ),
    (
        "Клыки в стену",
    ...
    (
        "Двойник портала",
        "Войти в портал, который ведёт к двойнику одного из стаи, — и заменить его собой.",
    ...
    (
        "Последний страж",
        "Разбудить последнего стража портала, чтобы он пропустил стаю, — но страж потребует имя.",
    ...
    (
        "Дыхание портала",
        "Вдохнуть воздух портала и задержать дыхание, пока стая не минует порог — мир начнёт задыхаться.",
    ...
    (
        "Хвост за порогом",
        "Оставить хвост за порогом портала — он упадёт в чужой мир и станет ключом.",
...
_CARE_PATHS = [
    ...
    (
        "Последний вздох",
        "Выдохнуть воздух портала за стаю, приняв весь яд на себя, — мир отдохнёт на минуту.",
        "Яд вышел. Стая жива. Но стражник portals теперь считает дни до следующего вдоха.",
    ),
    (
        "Сон для двоих",
...
_CUNNING_PATHS = [
    ...
    (
        "Два портала",
        "Открыть два портала одновременно и войти в третий, которого не видят датчики.",
        "Третий портал открыт. Датчики молчат. Стая вошла в мир, который не существует на карте.",
    ),
    (
        "Чужой запах",
        "Натереться пылью чужого портала, чтобы датчики приняли стаю за своих.",
...
def compose_chapter(
    ...
    def _dog_pads(day: int) -> tuple[str, ...]:
        ...
        pixel = {
            ...
            "mid": "Пиксель-Оккультист поймал искру из портала и принёс её в зубах — та потухла у миски.",
        ...
        nameless = {
            ...
            "late": "Безымянная-Варлок стоит у портала и не входит: она слышит то, что стая ещё не услышала.",
        }
        _EARLY_ATMOSPHERIC = (
            ...
            "Портал в углу мигнул раз за разом — один, два, три — и замолчал, как считал.",
            "Тень Баркода легла на карту длиннее, чем он сам. Она решала за него.",
            "Чужой голос прошептал имя стаи и растаял на середине.",
            "Запах дождя пришёл из portals, но дождя нигде не было.",
            ...
            "Тепло от стенки портала обожгло нос — тот же портал был холодным секунду назад.",
        ...
        _MID_ATMOSPHERIC = (
            ...
            "Портал вспыхнул не в свой черёд — стая заметила, но промолчала.",
            ...
            "Искра из портала упала на карту и оставила дырку размером с сердце.",
        ...
        _LATE_ATMOSPHERIC = (
            ...
            "Портал не открыл дверь. Он просто показал тьму — и тьма была тёплой.",
            ...
            "Портал мигнул в последний раз — и стая увидела в нём своё отражение.",
            ...
            "Холод пронизал насквозь — не от портала, а от тишины, которая заполнила пустоту.",
            ...
            "Горячий след на полу тянулся к порталу — но портал был холодным.",
        )
        atmospheric = {
...
_MIDTWIST_POOL = (
    ...
    "Стена портала слева дала трещину, и из неё потянуло холодом — не зимним, "
    ...
    "Портал замер в положении полуоткрытости: он не закрылся и не открылся — "
