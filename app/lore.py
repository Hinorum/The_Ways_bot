...
# «Архив Начала» — вступление-предыстория голосом дневника. Это первая
# страница канона: её читают новичку в /lore до того, как начинаются тропы.
# Голос лабиринта, без восклицаний, без точки в конце — как и положено.
# Дневник — не торговец, а проводник, который выбирает, какой коридор открыть.
ARCHIVE_ORIGIN = (
    "📜 <b>Архив Начала</b>\n"
...
_CUNNING_PATHS = [
    ...
    (
        ...
        "Печать выдержала проверку. Но дневник знает чужой почерк — и молчит.",
    ),
    (
        "Двойной аргумент",
    ...
    (
        ...
        "Мир поверил. Стая прошла мимо. Но дневник записал: запись — не голос.",
    ),
    (
        "Два коридора",
    ...
    (
        "Поддельный след",
        "Оставить ложный след, который поведёт дневник в сторону, — пока стая идёт к цели.",
        "Дневник пошёл по ложному следу. Стая добралась. Но ложный след остался в дневнике — навсегда.",
    ),
    (
        "Тихий вор",
    ...
    (
        ...
        "Имя принято. Страж пропустил. Но ложное имя теперь в папке — и дневник его найдёт.",
    ),
    (
    ...
    (
        ...
        "Закон подменён. Голоса пошли не туда. Но дневник видит расхождение — и он им торгует.",
    ),
    (
        "Создать хаос",
...
def _law_voice(win_rule) -> str:
    ...
    voices = {
        WinRule.MAJORITY: (
            "«Страница перевернулась… многие лапы прошли по ней… но запись помнит иначе», — "
...
_SEAL_VOICE = (
    "«Сегодня лабиринт запечатал дверь до вечера», — шепчет Старый дневник, "
    "и в его шёпоте слышна улыбка. «Какой сегодня коридор — узнаете вместе с итогами. "
    "Если доживёте до итогов». Правило дня скрыто: стая гадает, спорит и принюхивается. "
)
...
_NPC_REACTIONS = {
    "risk": (
        ...
        ("Пиксель", "ловил лапой искры и молчал, считая тех, кто может не вернуться."),
    ...
    "care": (
        ...
        ("Стежка", "не тронула миску, пока не наелись все, — только потом своя."),
    ...
    "cunning": (
        ...
        ("Вектор", "не одобрил хитрость, но промолчал — стая сберегла лишнюю кость."),
...
async def _generate_atmospheric_via_llm(phase: str, llm_caller) -> list[str] | None:
    ...
    prompt = (
        ...
        f'["строка 1", "строка 2", ...]\n\n'
        f"Стиль: тёмный, атмосферный, метафоричный. Каждая строка — самостоятельная деталь."
    )
