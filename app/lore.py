...
# «Архив Начала» — вступление-предыстория голосом Смотрителя Троп. Это первая
...
# Смотритель Троп — не торговец, а проводник, который выбирает, какой коридор открыть.
ARCHIVE_ORIGIN = (
    "📜 <b>Архив Начала</b>\n"
...
def _law_voice(win_rule) -> str:
    """Закон дня репликой Смотрителя Троп — даже без нейросети.
    ...
    Смотритель Троп — хозяин лабиринта, выбирает какой коридор открыть.
    """
    ...
    voices = {
        WinRule.MAJORITY: (
            ...
            "говорит Смотритель Троп и поворачивает ключ. «Громкие лапы "
            "ведут — а фонарь покажет, кто молчал». "
        ),
        WinRule.MINORITY: (
            "«Сегодня я выбрал узкий коридор», — шепчет "
            "Смотритель Троп. «Меньше голосов — сильнее след. Не "
            "благодарите — это не подарок, а мой выбор». "
        ),
        WinRule.MEDIAN: (
            "«Середина знает меру — но я показываю обе крайности», — роняет "
            "Смотритель Троп и поворачивает ключ ровно посередине. "
            "«Крайности пусть подождут… фонарь покажет завтра». "
        ),
    }
