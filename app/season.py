...
def finale_instruction(
    ...
    Тайна счёта замыкается здесь же: весь сезон пересчёт выходил на единицу
    короче, чем собак, — потому что пятой в списке нет той, кому нечем
    назваться. День Первого Лая — шанс назвать её следом без имени; стая
    может закрыть счёт или уйти с открытым. Решает сама, цифры не трогаем.
    """
    ...
    if vow_count:
        vow_line = (
            ...
            "будет выкуплена этим финалом — или что останется невыкупленным навсегда."
        )
    whole_line = ""
    if healed_memories:
        whole_line = (
    ...
    recount_line = (
        " Весь сезон счёт выходил ровно на единицу короче, чем собак; сегодня "
        "позади стен тишина — пересчёт кончился и ждёт. Пятая в списке так и не "
        "появилась: не потому, что Администратор не смог её найти, а потому, что "
        "ей нечем назваться. Дай стае встретиться с этим: назвать безымянный след "
        "своим именем — или стоять перед ним честно. Счёт закрывается только тем, "
...
def exodus_instruction(
    ...
    if phase == 2:
        ...
        if healed_memories:
            base += (
                f" Из пятерых память приняли {healed_memories}/5: принявшие её с собой "
