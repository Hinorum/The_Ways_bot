...
def _llm_json_value(text: str) -> dict | list | None:
    ...
    Идентично прошлому `story._extract_json` на основном пути: raw_decode
    понимает кавычки/экранирование и переживает текст до/после JSON.
    """
    stripped = text.strip()
    ...
    if start == -1:
        return None
    try:
        value = json.JSONDecoder().raw_decode(stripped[start:], 0)[0]
    except json.JSONDecodeError:
        return None
