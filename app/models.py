...
RULE_PHRASES = {
    WinRule.MAJORITY: "уцелеет кадр с большей суммой Gram (день Грома)",
    WinRule.MINORITY: "уцелеет кадр с меньшей суммой Gram (день Шёпота)",
    WinRule.MEDIAN: "уцелеет кадр со средним весом Gram (день Середины)",
}
...
VOTE_RULE_PHRASES = {
    WinRule.MAJORITY: "уцелеет кадр, собравший больше всех голосов",
    WinRule.MINORITY: "уцелеет кадр, собравший меньше всех голосов",
    WinRule.MEDIAN: "уцелеет кадр со средним числом голосов",
}
...
class Referral(Base):
    ...
    навсегда, повторные /start с чужой ссылкой игнорируются. Самоподписка и
    циклы цепочки (A привёл B, B приводит A обратно) блокируются в
    record_referral. Размен между собственными альтами не различим на уровне
    кошельков — осознанный риск реферальных схем, ограниченный долей
    referral_pct (см. app/referrals.py). Награды копятся в ReferralPot
    (см. там) и выплачиваются через обычную очередь Payout.
    """
...
class Stake(Base):
    """Ставка TON на сцену дня. Одна на игрока в раунде, идемпотентна по tx_hash.
...
class MemoryHit(Base):
    ...
    счётчик «Память пути», видимый в /score. Одна отметка на игрока в день.
    """
