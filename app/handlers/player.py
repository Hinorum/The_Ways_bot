...
async def cmd_start(message: Message) -> None:
    ...
    lines = [
        ...
        "",
        "Город вокруг Вокзала: поезда ушли в Перелог, и в тумане",
        "остались только собаки — разумные и очень внимательные.",
        "Ты — один из стаи.",
        "",
        "Каждый день месяца — станция пути. Состав ждёт на Перроне №0,",
        "и стая у котла выбирает тропу. Закон дня выпадает честной",
        "энтропией: большинство, меньшинство или середина.",
        "",
        "Тропу решают ставки Gram — бросок грамма в котёл дня.",
        "Банки скрыты до итогов: мир решает молча и впечатает",
        "победившую тропу в завтрашнюю станцию.",
    ]
    if settings.ton_enabled:
        lines.append(
            "🐾 Как ставить — /stake. Голос без ставки тоже ведёт тебя: "
            "он строит лидерборд недели и месяца."
        )
    else:
        lines.append("🐾 Тропу выбирают кнопкой под картами — до закрытия дня.")
    lines += [
        "",
