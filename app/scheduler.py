...
async def _compose_whisper(
    ...
    try:
        ...
        if echo is not None:
            ...
            }.get(echo.kind, "лабиринт гудит не в такт")
    ...
    messages = [
        {"role": "system", "content": DM_SYSTEM_PROMPT},
        {
            ...
            "content": (
                ...
                + cards_line
                + task
                + "Пиши от третьего лица, настоящее время. "
...
_WHISPER_FALLBACKS = (
    ...
    "Дневник захлопнулся ровно на середине. «До утра всё может стать иначе», — шепнул он.",
    "Миски остыли, не тронутые. Баркод смотрел то на одну карту, то на другую, будто ждал подсказки.",
    "Безымянная встала мордой к стене и не двинулась до темноты. Собаки чувствуют развилки кожей.",
    "Стены лабиринта загудели ниже обычного — почти шёпотом. Так лабиринт прислушивается к ещё не принятым решениям.",
    ...
    "Администратор сегодня не считал вслух. Тишина от него страшнее любого счёта.",
    "Из подвала доносилось шипение: «Этот выбор котировался на 47%… ошибка рынка…».",
    "У стены лежала обглоданная табличка — номер вне каталога, но кто-то упорно её грыз.",
...
_WHISPER_OPENERS = (
    ...
    "как нетерпеливый лабиринт, и в этом молчании есть что-то слишком внимательное.",
)
