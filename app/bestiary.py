...
- сюжетные звери сезона (Хозяин Ошибки, Еретик, кот, Чинитель, Совет,
  руины, огонь) — появляются, когда их ступень плана перестаёт быть бытовой;
- силы сансары (Крыса, Анубис) — приходят по позиции в сезоне: Крыса
...
BEASTIES: dict[str, tuple[str, str]] = {
    ...
    "cat": (
        ...
        "След не последний: старый мир вернётся за стаей — после "
        "технических работ. SOON.",
    ),
    "engineer": (
        "Чинитель порогов",
        "Старый мастер порталов из того мира, где был один сон на всех. "
        "Он чинит порталы так, что те помнят, кого провели, — и ломает "
        "их так, что этого никто не узнаёт. В кризисе он единственный, "
        "кто умеет закрыть коридор Хозяина Ошибки с чужой стороны.",
    ),
    "chingiz": (
        "Двое с перехода",
        "Двое из старого свода, что держали тактику переходов между "
        "мирами: воин поминовения и полководец мисок. В стае о них "
        "говорят шёпотом: их появление значит, что старый мир уже на "
        "полпути к порогам Сети.",
    ),
    "dogtown": (
        "Руины первого города",
        "То, что осталось от первого приюта стаи, — пустоши хранят его "
        "каркас, а рынок над рекой торгует его памятью. Когда стая видит "
        "руины, это значит: карты описывают не только будущее, но и то, "
        "что уже было.",
    ),
    "aretha": (
        "Поджигательница",
        "Персонификация риска и огня из старого мира: палит проводку, "
        "будит порталы и не признаёт чужих правил. Её тёплый след "
        "перекликается с тёплыми мисками стаи — но она выбирает гореть, "
        "а не греться.",
    ),
    "rat": (
        "Крыса из подвала Сети",
        "Живёт в стёртых версиях дней, тех, что «не случились». Помнит все "
        "круги сансары и продаёт дежавю-подсказки за память: «забота сегодня "
        "недооценена», «хитрость уже выкупили». Счёт её не стирает: никто не "
        "считает её за ценность.",
    ),
    "anubis": (
        "Анубис, Судья Цикла",
...
async def note_round(session: AsyncSession, round_row: Round, season_key_value: str | None = None) -> int:
    ...
    try:
        ...
        if run_day <= 4:
            # В первые дни круга сансара напоминает о себе: Крыса подвала.
