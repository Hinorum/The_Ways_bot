...
def rule_block_ref(round_row) -> str:
    ...
    «seqno:root_hash» энтропии закона → « (блок TON №seqno, проверяемо в
    эксплорере)». Пусто — закон выпал локальным жребием (TON выключен/падал
    при открытии дня), и игроки видят просто закон без ссылки на блок.
    """
    entropy = getattr(round_row, "rule_entropy", None)
    if entropy and ":" in entropy:
        seqno = entropy.split(":", 1)[0]
        return f" (блок TON №{seqno}, проверяемо в эксплорере)"
    return ""
