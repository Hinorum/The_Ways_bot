...
def test_character_motifs_detected_from_chapter_text() -> None:
    ...
    text = "Лайнер отсчитывает сдачу, а дневник шепчет над страницами. Администратор молчит."
    motifs = character_motifs_for(text)
    assert len(motifs) >= 2  # Лайнер + дневник + Администратор (дедуп)
    assert all("liner" in m.lower() or "journal" in m.lower() or "administrator" in m.lower() for m in motifs)
