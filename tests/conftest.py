...
# Тесты герметичны: локальный .env (напр. Testnet) не должен менять результат.
# Сеть и режим денег фиксируем до импортов app.* — current_network() и фильтр
# ставок в итогах дня стабильны независимо от машины: фикстуры создают Stake
# с дефолтной сетью моделей ("mainnet"), а большинство тестов ожидает
# бесплатную версию и включает ton_enabled=True точечно через monkeypatch.
os.environ["TON_NETWORK"] = "mainnet"
os.environ["TON_ENABLED"] = "false"
